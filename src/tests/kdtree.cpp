...
void CheckKdSceneAverage(const std::string &filename, float expected) {
    Point2i resolution;
...
std::vector<TestScene> GetKdTreeScenes() {
...

    {
        ...
        std::shared_ptr<KdTreeAccel> kdtree = std::make_shared<KdTreeAccel>(prims);

        std::vector<std::shared_ptr<Light>> lights;
        lights.push_back(
            std::make_shared<PointLight>(Transform(), nullptr, Spectrum(Pi)));

        std::unique_ptr<Scene> scene(new Scene(kdtree, lights));
...
std::vector<std::pair<std::shared_ptr<Sampler>, std::string>> GetKdTreeSamplers(
...
struct KdTreeRenderTest : public testing::TestWithParam<TestIntegrator> {};

TEST_P(KdTreeRenderTest, RadianceMatches) {
    Options options;
    options.quiet = true;
    pbrtInit(options);
    ...
    CheckKdSceneAverage(inTestDir("test.exr"), tr.scene.expected);
...
INSTANTIATE_TEST_CASE_P(KdTree, KdTreeRenderTest,
                        testing::ValuesIn(GetKdTreeIntegrators()));
