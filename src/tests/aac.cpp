...
void CheckAACSceneAverage(const std::string &filename, float expected) {
    Point2i resolution;
...
std::vector<TestScene> GetAACScenes() {
...

    {
        ...
        std::shared_ptr<AACAccel> kdtree = std::make_shared<AACAccel>(prims, 20, 0.1);
...
std::vector<std::pair<std::shared_ptr<Sampler>, std::string>> GetAACSamplers(
...
struct AACRenderTest : public testing::TestWithParam<TestIntegrator> {};

TEST_P(AACRenderTest, RadianceMatches) {
    Options options;
    options.quiet = true;
    options.nThreads = 1;
    pbrtInit(options);
    ...
    CheckAACSceneAverage(inTestDir("test.exr"), tr.scene.expected);
...
INSTANTIATE_TEST_CASE_P(AAC, AACRenderTest,
                        testing::ValuesIn(GetAACIntegrators()));
